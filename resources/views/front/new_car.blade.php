    ...
    <div id="HomePage">
        ...
        <div class="card">
            <div class="card-body" style="padding:0px;">
                <h5 class="card-title CT1"><i class="fa fa-car"></i> سيارات جديدة</h5>
                <h6 class="text-muted desc-textpg">في هذا القسم نركز على عرض الموديلات الجديدة من السيارات المتاحة للشراء
                    نقداً أو بالتقسيط وفقا لمواصفاتها الأساسية واسعارها حيث العديد من السيارات المضافة يومياً والتي يقوم
                    فريق الموقع بإدخالها وتحديث كافة بياناتها</h6>
            </div>
        </div>
        <div class="PageCard">
            ...
            <div class="card-body" style="padding:0px;">
                ...
                <div class="rgt card-text showRight postList" style="padding:5px;">
                    <div id="ads-container">
                        @include('front.partials.ads', ['lastadvs' => $lastadvs])
                    </div>
                    <div class="clr"></div>
                    ...
                    <script>
                        $(document).ready(function() {
                            $('#load-more').on('click', function() {
                                let page = $(this).data('page'); // الحصول على رقم الصفحة
                                $('#loading').show(); // إظهار رسالة التحميل
                                $.ajax({
                                    url: '/new-cars?page=' + page, // إرسال طلب AJAX
                                    type: 'GET',
                                    success: function(response) {
                                        if (response.trim() === '') {
                                            // إذا لم تكن هناك بيانات إضافية
                                            $('#load-more').hide(); // إخفاء الزر
                                        } else {
                                            $('#ads-container').append(response); // إضافة الإعلانات الجديدة
                                            $('#load-more').data('page', page + 1); // تحديث رقم الصفحة
                                        }
                                        $('#loading').hide(); // إخفاء رسالة التحميل
                                    },
                                    error: function() {
                                        alert('حدث خطأ أثناء تحميل المزيد من الإعلانات.');
                                        $('#loading').hide();
                                    }
                                });
                            });
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('car.search') }}" method="GET" class="form-search">
                                ...
                                <button type="submit" name="search"
                                    class="btn btn-primary btn-block main_seach_button"><i class="fas fa-search"></i>
                                    البحث </button>
                            </form>
                        </div>
                    </div>
